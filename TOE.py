...
FRENCHYEAR={'01':'janvier','02':'fevrier','03':'mars','04':'avril','05':'mai','06':'juin','07':'juillet','08':'aout','09':'septembre','10':'octobre','11':'novembre','12':'decembre'}
YEAR={'01':'Jan','02':'Feb','03':'Mar','04':'Apr','05':'May','06':'Jun','07':'Jul','08':'Aug','09':'Sep','10':'Oct','11':'Nov','12':'Dec'}
...
class WrongValue():
	""" exception raises when the value is wrong """
...
def get_env_var(varname):
	...
	if not variablename :
		raise MissingValue("\nvalue not defined, la variable d'environnement %s est manquante\n" %varname)
	else :
		return variablename	
...
def reconversion(string):
	...
	day=string[3:5]
	for key, value  in FRENCHYEAR.items():
...
#change time format "twitter" to "normal" : Thu Jun 26 07:40:58 +0000 2014 -> 2014-06-26T10:55:42
...
def DateRecovery(status,data,size,i):
	...
	#var to cover the list, possible= true if we find a "readable" tweet, nb event since the last tweet "readable"
	t=0
	possible=True
	nbEvent=0
...
#recover the last tweet date, even if its not a earthquake and recover all event from this date
...
#compare two date, return bool (== <=> false) date format type 2014-06-26T10:55:42
...
if __name__ == '__main__' :
	...
	sock = urllib.urlopen("http://renass.unistra.fr/fdsnws/event/1/query?orderby=time&format=json&longitude=1.9&limit=%s&latitude=46.6&maxradius=8.0" %nb)
	text = sock.read()
	sock.close()
	...
	#if possible :
	print '\nDernier evenement publie: ',date,'\nNombre d\'evenement(s) depuis :', nbEvent,'\n'
